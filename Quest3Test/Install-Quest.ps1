...
if ($apks.Count -ne 1) { throw 'Expected one packaged test APK. Run Build-Quest.ps1 first.' }
$apk = $apks[0].FullName
...
if ($LASTEXITCODE -ne 0 -or !($metadata | Select-String "^package: name='$package'")) {
    throw 'APK does not identify itself as the isolated ACE Quest test.'
}
if (!$SkipData) {
...
if (!$NoLaunch) {
    ...
    if ($powerState -match 'mWakefulness=Awake') {
        ...
        Write-Host 'Headset is asleep. Put it on and open ACE Quest Test from Apps > Unknown Sources.'
    }
}
Write-Host "Installed ACE Quest Test. Default server: 10.0.0.26:9000. DAT path: $saved/DAT"
