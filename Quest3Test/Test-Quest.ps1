...
$remoteSaved = 'files/ACEViewer/Saved'
$absoluteSaved = "/data/user/0/$package/$remoteSaved"
$argsText = '-UserDir=/data/user/0/com.acecommunity.questtest/files/ACEViewer/ -ExecCmds="Automation RunTests ACE.Quest.NativeSmoke+ACE.Packaging.Materials+ACE.Network.LoginHandshake+ACE.RetailParity.LocalLoginSettings+ACE.VR.GesturesAndSettings+ACE.Packaging.TextureBudget+ACE.Rendering.SceneAudit" -ReportExportPath=' + $absoluteSaved + '/Automation/' + $reportName
...
if ($result.failed -gt 0 -or ($result.succeeded + $result.succeededWithWarnings) -lt 7 -or $result.notRun -gt 0 -or $result.inProcess -gt 0) {
