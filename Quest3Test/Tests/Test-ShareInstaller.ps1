...
$fixtureApk = Join-Path $workspace 'ACEViewer-arm64.apk'
'Test-only fixture, never installed on an Android device.' | Set-Content -LiteralPath $fixtureApk -Encoding ASCII
@{
    package='com.acecommunity.questtest'; version='test'; apk='ACEViewer-arm64.apk'
...
if (($args -join ' ') -match 'shell run-as com.acecommunity.questtest test -s files/ACEViewer/Saved/DAT/') { exit 0 }
$cmd = $args -join ' '
if ($scenario -eq 'data-repair') {
    ...
    if ($cmd -match 'shell am force-stop com.acecommunity.questtest$|shell run-as com.acecommunity.questtest mkdir -p files/ACEViewer/Saved/DAT$') { exit 0 }
    if ($cmd -match 'if \[ -f files/ACEViewer/Saved/DAT/(client_\w+\.dat) \]') {
...
function Test-Case {
    ...
    if ($commands -match '\buninstall\b|\bkill-server\b|\bclear\b' -or ($Scenario -ne 'data-repair' -and $commands -match '\bforce-stop\b')) { throw "Case $Scenario used a destructive or unrelated command" }
...
Write-Output "All 14 installer cases passed under Windows PowerShell. Evidence: $workspace"
