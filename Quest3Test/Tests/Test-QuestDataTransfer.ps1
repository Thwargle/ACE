...
public class FakeAdb {
 public static int Main(string[] args) {
  ...
  var match = Regex.Match(args[4], @"cat > files/ACEViewer/Saved/DAT/(client_\w+\.dat\.installing)'$");
...
function Invoke-Adb { param([string[]]$Arguments)
    $cmd = $Arguments -join ' '
    $script:commands += $cmd
    if ($cmd -match 'shell am force-stop com.acecommunity.questtest$|shell run-as com.acecommunity.questtest mkdir -p files/ACEViewer/Saved/DAT$') { return }
    ...
    if ($cmd -match 'stat -c %s files/ACEViewer/Saved/DAT/(client_\w+\.dat\.installing)$') {
        return [string](Get-Item -LiteralPath (Join-Path $deviceDir $Matches[1])).Length
    }
    if ($cmd -match 'sha256sum files/ACEViewer/Saved/DAT/(client_\w+\.dat\.installing)$') {
        return (Get-FileHash -LiteralPath (Join-Path $deviceDir $Matches[1]) -Algorithm SHA256).Hash.ToLowerInvariant() + '  file'
    }
    if ($cmd -match 'mv files/ACEViewer/Saved/DAT/(client_\w+\.dat\.installing) files/ACEViewer/Saved/DAT/(client_\w+\.dat)$') {
