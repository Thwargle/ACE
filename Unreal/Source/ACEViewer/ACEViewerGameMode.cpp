...
namespace ACEViewerWorldComposition
{
...
AACEViewerGameMode::AACEViewerGameMode()
{
	...
	DefaultPawnClass = AACEViewerPawn::StaticClass();
...
void AACEViewerGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
	Super::InitGame(MapName, Options, ErrorMessage);
	ApplyLoginStreamingSetup();
}

void AACEViewerGameMode::PreInitializeComponents()
{
	Super::PreInitializeComponents();
	ApplyLoginStreamingSetup();
}

void AACEViewerGameMode::ApplyLoginStreamingSetup()
{
	if (UWorld* World = GetWorld())
	{
		...
		if (bWcMap || bWpMap)
		{
			ACEViewerWorldComposition::RelocatePlayerStartsForLogin(World);
		}
	}
}

APawn* AACEViewerGameMode::SpawnDefaultPawnFor_Implementation(AController* NewPlayer, AActor* StartSpot)
{
	...
	if (bWcMap || bWpMap)
	{
		TSubclassOf<APawn> PawnClass = DefaultPawnClass ? DefaultPawnClass : TSubclassOf<APawn>(AACEViewerPawn::StaticClass());
...
void AACEViewerGameMode::StartPlay()
{
	Super::StartPlay();
	...
	UE_LOG(LogTemp, Warning, TEXT("ACE: AACEViewerGameMode::StartPlay — DefaultPlayerControllerClass=%s"),
	...
	ACEViewerWorldComposition::HideLoadedWcLandscapes(GetWorld());
	...
	GetWorldTimerManager().SetTimer(ForceLoginUITimerHandle, this, &AACEViewerGameMode::TryForceShowLoginUI, 0.25f, true);

	if (bHideTemplateLevelActors)
	{
		GetWorldTimerManager().SetTimerForNextTick(this, &AACEViewerGameMode::HideConflictingTemplateActors);
	}
...
void AACEViewerGameMode::Tick(float DeltaSeconds)
{
...
void AACEViewerGameMode::HideConflictingTemplateActors()
{
	UWorld* World = GetWorld();
	if (!World)
	{
...
void AACEViewerGameMode::EnsureAceSkySpawned()
{
	if (!bSpawnAceSky)
	{
		return;
...
void AACEViewerGameMode::SuppressConflictingLevelSkyLights()
{
	UWorld* World = GetWorld();
	if (!World)
	{
...
void AACEViewerGameMode::TryForceShowLoginUI()
{
