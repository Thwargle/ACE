...
bool FACEVRInputLifecycleTest::RunTest(const FString&)
{
    ...
    auto Added=Make(TEXT("NewDynamicRow"),0,0,10,10);Other->AddChild(Added);
    ...
    Added->SetElementName(TEXT("RenamedDynamicRow"));
    Manager->BeginNameLookupPass();
    TestFalse(TEXT("Renamed row loses its old cached name"),Manager->FindElementByName(TEXT("NewDynamicRow")).IsValid());
    TestTrue(TEXT("Renaming between frames invalidates membership"),Manager->FindElementByName(TEXT("RenamedDynamicRow"))==Added);
    Added->SetElementName(TEXT("NewDynamicRow"));
    TestTrue(TEXT("Renaming inside a lookup pass is immediate"),Manager->FindElementByName(TEXT("NewDynamicRow"))==Added);
    Window->AddChild(Added);
    TestFalse(TEXT("Reparenting removes old ancestor membership"),Manager->FindElementUnder(TEXT("OtherWindow"),TEXT("NewDynamicRow")).IsValid());
