...
static TAutoConsoleVariable<int32> GACECachedWorldDraws(TEXT("ace.Render.CachedWorldDraws"), 1,
	TEXT("Cache terrain/room draw batches. Set before loading the scene; 0 uses the reference dynamic path."), ECVF_RenderThreadSafe);
...
public:
		...
		, bCachedDraws(Component->bPreferCachedDraws && GACECachedWorldDraws.GetValueOnGameThread() != 0)
	{
		...
		for (int SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
		{
			...
			if (SrcSection.ProcIndexBuffer.Num() > 0 && SrcSection.ProcVertexBuffer.Num() > 0)
			{
				...
				NewSection->RenderIndexCount = SrcSection.GetRenderIndexCount();

				// Save ref to new section
				Sections[SectionIdx] = NewSection;

#if RHI_RAYTRACING
				if (IsRayTracingEnabled() && NewSection->RenderIndexCount > 0)
				{
						...
						[this, DebugName = Component->GetFName(), NewSection](FRHICommandListImmediate& RHICmdList)
					{
						...
						Initializer.TotalPrimitiveCount = NewSection->RenderIndexCount / 3;
	...
	void UpdateSection_RenderThread(FRHICommandListBase& RHICmdList, FProcMeshSectionUpdateData* SectionData)
	{
		...
		if(	SectionData != nullptr) 			
		{
				...
				Sections[SectionData->TargetSection] != nullptr)
			{
				...
				Section->RenderIndexCount = SectionData->RenderIndexCount;
				...
				if (NumVerts > 0)
				{
					...
					const uint32 Bytes = NumVerts * VertexBuffer.GetStride();
					void* VertexBufferData = RHICmdList.LockBuffer(VertexBuffer.VertexBufferRHI, 0, Bytes, RLM_WriteOnly);
					FMemory::Memcpy(VertexBufferData, VertexBuffer.GetVertexData(), Bytes);
				...
				if (NumVerts > 0)
				{
					...
					const uint32 Bytes = (VertexBuffer.GetTangentSize() / VertexBuffer.GetNumVertices()) * NumVerts;
					void* VertexBufferData = RHICmdList.LockBuffer(VertexBuffer.TangentsVertexBuffer.VertexBufferRHI, 0, Bytes, RLM_WriteOnly);
					FMemory::Memcpy(VertexBufferData, VertexBuffer.GetTangentData(), Bytes);
					RHICmdList.UnlockBuffer(VertexBuffer.TangentsVertexBuffer.VertexBufferRHI);
				}

				if (NumVerts > 0 && SectionData->bUpdateUVs)
				{
					auto& VertexBuffer = Section->VertexBuffers.StaticMeshVertexBuffer;
				...
				{
					Section->RayTracingGeometry.ReleaseResource();
					if (Section->RenderIndexCount > 0)
					{
						FRayTracingGeometryInitializer Initializer;
						Initializer.IndexBuffer = Section->IndexBuffer.IndexBufferRHI;
						Initializer.TotalPrimitiveCount = Section->RenderIndexCount / 3;
						Initializer.GeometryType = RTGT_Triangles;
						Initializer.bFastBuild = true;
						Initializer.bAllowUpdate = false;
	...
	{
		if (!bCachedDraws) return;
		PDI->ReserveMemoryForMeshes(Sections.Num());
		for (int32 Index = 0; Index < Sections.Num(); ++Index)
		{
			const FProcMeshProxySection* Section = Sections[Index];
			if (!Section || !Section->bSectionVisible || Section->RenderIndexCount == 0) continue;
			FMeshBatch Mesh;
			...
			Mesh.SegmentIndex = Index;
			Mesh.MeshIdInPrimitive = Index;
			Mesh.LODIndex = 0;
			auto& Element = Mesh.Elements[0];
			Element.IndexBuffer = &Section->IndexBuffer;
			Element.PrimitiveUniformBuffer = GetUniformBuffer();
			Element.FirstIndex = 0;
			Element.NumPrimitives = Section->RenderIndexCount / 3;
			Element.MinVertexIndex = 0;
			Element.MaxVertexIndex = Section->VertexBuffers.PositionVertexBuffer.GetNumVertices() - 1;
	...
	virtual void GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const override
	{
		...
		{
			if (Section != nullptr && Section->bSectionVisible && Section->RenderIndexCount > 0)
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
				{
					if (VisibilityMap & (1 << ViewIndex))
					{
						...
						if (GACEReusePrimitiveBuffers.GetValueOnRenderThread() && GetUniformBuffer())
						{
							...
							// buffers for every visible AC body part and room each frame.
						...
						BatchElement.NumPrimitives = Section->RenderIndexCount / 3;
	...
	virtual FPrimitiveViewRelevance GetViewRelevance(const FSceneView* View) const
	{
		...
		const bool bDebugDraw = AllowDebugViewmodes() && View->Family->EngineShowFlags.Wireframe;
		Result.bDynamicRelevance = !bCachedDraws || bDebugDraw;
		Result.bStaticRelevance = bCachedDraws && !bDebugDraw;
...
void UProceduralMeshComponent::CreateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FColor>& VertexColors, const TArray<FProcMeshTangent>& Tangents, bool bCreateCollision)
{
	...
	{
		UE_LOGF(LogProceduralComponent, Warning, "Detected %d degenerate triangle%ls with non-unique vertex indices for created mesh section in '%ls'; degenerate triangles will be dropped.",
...
void UProceduralMeshComponent::UpdateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FColor>& VertexColors, const TArray<FProcMeshTangent>& Tangents)
{
	...
	if(SectionIndex < ProcMeshSections.Num())
	{
		...
		if (bSameVertexCount)
		{
			Section.RenderIndexCount = INDEX_NONE;
			...
			if (SceneProxy && !IsRenderStateDirty())
			{
				...
				SectionData->RenderIndexCount = Section.GetRenderIndexCount();
		...
		{
			UE_LOGF(LogProceduralComponent, Error, "Trying to update a procedural mesh component section with a different number of vertices [Previous: %i, New: %i] (clear and recreate mesh section instead)", PreviousNumVerts, NumVerts);
		}
	}
}
	...
	TConstArrayView<FColor> Colors, int32 ActiveIndexCount)
{
	...
	if (Section.bEnableCollision || Count > Section.ProcVertexBuffer.Num()
		|| Normals.Num() != Count || Colors.Num() != Count || ActiveIndexCount < 0
		|| ActiveIndexCount > Section.ProcIndexBuffer.Num() || ActiveIndexCount % 3 != 0
		|| (ActiveIndexCount > 0 && Count == 0)) return false;
#if DO_GUARD_SLOW
	for (int32 I = 0; I < ActiveIndexCount; ++I) checkSlow(Section.ProcIndexBuffer[I] < uint32(Count));
#endif
	...
	if (SceneProxy && !IsRenderStateDirty())
	{
		auto* Data = new FProcMeshSectionUpdateData;
		Data->TargetSection = SectionIndex;
		Data->NewVertexBuffer.Append(Section.ProcVertexBuffer.GetData(), Count);
		Data->RenderIndexCount = ActiveIndexCount;
		Data->bUpdateUVs = false;
		auto* Proxy = static_cast<FProceduralMeshSceneProxy*>(SceneProxy);
		ENQUEUE_RENDER_COMMAND(FProcMeshPrefixUpdate)([Proxy, Data](FRHICommandListImmediate& RHICmdList)
		{
			Proxy->UpdateSection_RenderThread(RHICmdList, Data);
		});
	}
	UpdateLocalBounds();
...
void UProceduralMeshComponent::SetMeshSectionVisible(int32 SectionIndex, bool bNewVisibility)
{
	if(ProcMeshSections.IsValidIndex(SectionIndex))
	{
		...
		if (ProcMeshSections[SectionIndex].bSectionVisible == bNewVisibility) return;
		ProcMeshSections[SectionIndex].bSectionVisible = bNewVisibility;
		if (bPreferCachedDraws)
		{
			MarkRenderStateDirty();
			return;
		}
