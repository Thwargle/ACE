...
public:
	...
	/** ACE runtime particles reserve capacity but render only the active index prefix. */
	int32 RenderIndexCount = INDEX_NONE;
	int32 GetRenderIndexCount() const { return RenderIndexCount < 0 ? ProcIndexBuffer.Num() : FMath::Min(RenderIndexCount, ProcIndexBuffer.Num()); }
...
public:
	...
	/** Cache draw batches for geometry that changes only when streamed/rebuilt.
	 * Set before registration. Section edits invalidate the cached batches.
	 * Particle buffers and animated geometry retain the dynamic path by default. */
	UPROPERTY()
