# Package launcher template. Copy next to the packaged ACEViewer.exe.
$ErrorActionPreference = 'Stop'
$executable = Join-Path $PSScriptRoot 'ACEViewer.exe'
if (!(Test-Path -LiteralPath $executable)) { throw 'Keep this launcher beside ACEViewer.exe in the complete Windows package.' }
$gameExecutable = Join-Path $PSScriptRoot 'ACEViewer/Binaries/Win64/ACEViewer.exe'
if (!(Test-Path -LiteralPath $gameExecutable)) { throw 'The Windows package is incomplete. Keep its ACEViewer and Engine subfolders together.' }
if (Get-Process ACEViewer -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $gameExecutable }) {
...
$logDirectory = Join-Path $env:LOCALAPPDATA 'ACEViewer/Saved/Logs'
